%% Script to reproduce plots in Figure 3 and 4A-4B

clear all; close all;

% Load simulation results
PATH_static = ['path_to_main_directory\results\static_connectivity\']; % path to directory where the static connectivity results have been saved
PATH_dynamic = ['path_to_main_directory\results\dynamic_connectivity\']; % path to directory where the dynamic connectivity results have been saved

params.doOverlap = 1; % if 1 compute overlap between XCov and HOTE
params.nRep = 5;
minutes_range = [5 10 20 30 50 70 90 120 180];

for rIdx = 1:params.nRep
    for mIdx = 1:numel(minutes_range)
        filename = dir([PATH_static,'staticConnInference_rep',num2str(rIdx),'_min',num2str(minutes_range(mIdx)),'_*']);

        tmpFile = load([PATH_static,filename.name],...
...
save([PATH_static,'Overlap_XCov_HOTE_',date])

%% Distribution of synaptic weights (Fig3A and 3B)
filename_dynamic = dir([PATH_dynamic,'dynamicConnInference_rep1_min',num2str(minutes_range(mIdx)),'_*']);
load([PATH_dynamic,filename_dynamic],'gtDelay','s_time','minutes_range','Ne','M','staticConnMeasures');

prctiles = [1, 5, 10]; % percentiles to plot in Fig3A and Fig3B
...
%% Plot precision recall curve for different measures (panel 3C)
...
%% Plot AUPR with time for different measures (panel 3D)
...
%% Plot 5th and 10th prctile precision (panel 3E)
...
%% Compute fraction of inferred connections in all subgroups (panel 3F)
...
%% Plot fraction of connections per group comparing measures (panel 3F)
...
%% E/I estimation performance (Fig.4A and 4B)
cols = [0.4660 0.6740 0.1880;0.9290 0.6940 0.1250;0.4940 0.1840 0.5560;0 0.4470 0.7410;0.8500 0.3250 0.0980];
